...
"""
AutoGen Agent 客户端测试示例

此脚本测试 AutoGen Agent 服务器的多轮对话功能，
包括同步和流式请求，并且测试多次请求中的会话一致性。

使用方法:
1. 首先运行 autogen_agent_server.py 启动服务器
2. 然后运行此脚本: python autogen_agent_client_test.py
    ...
    for i, question in enumerate(test_questions):
        ...
        try:
            ...
            if hasattr(task_response.result, "response") and task_response.result.response:
                ...
                logger.info(f"代理响应: {response_text[:200]}...")  # 只显示前200个字符
...
async def test_streaming_conversation():
    ...
    for i, question in enumerate(test_questions):
        ...
        try:
            ...
            # 使用超时以避免无限等待
            timeout = 60.0  # 60秒超时 - AutoGen可能需要更长时间
                            ...
                            if hasattr(response, "parts") and response.parts:
                                for part in response.parts:
                                    if hasattr(part, "text") and part.text:
                                        ...
                                        logger.info(f"流式响应片段: {text_chunk[:30]}...")  # 只显示前30个字符
            ...
            logger.info(f"完整流式响应: {full_response_text[:200]}...")  # 只显示前200个字符
    ...
    for i, question in enumerate(session1_questions):
        ...
        try:
            ...
            if hasattr(task_response.result, "response") and task_response.result.response:
                ...
                logger.info(f"会话1 - 代理响应: {response_text[:200]}...")  # 只显示前200个字符
            else:
    ...
    for i, question in enumerate(session2_questions):
        ...
        try:
            ...
            if hasattr(task_response.result, "response") and task_response.result.response:
                ...
                logger.info(f"会话2 - 代理响应: {response_text[:200]}...")  # 只显示前200个字符
            else:
...
async def main():
    """主函数，运行所有测试"""
    logger.info(f"开始测试 AutoGen Agent 客户端 (服务器: {SERVER_URL})")
