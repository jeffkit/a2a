...
# 启动测试脚本
# 该脚本启动 OpenAI Function Agent 服务器并运行客户端测试

echo "===== OpenAI Function Agent 测试 ====="
...
if lsof -i:5003 &> /dev/null; then
    echo "警告: 端口 5003 已被占用，正在尝试关闭..."
    kill $(lsof -t -i:5003) 2>/dev/null || true
    sleep 2
fi

# 启动服务器（后台运行）
echo "启动 OpenAI Function Agent 服务器..."
...
# 检查服务器是否正在监听端口
for i in {1..10}; do
    if curl -s -o /dev/null http://localhost:5003; then
        echo "服务器已启动，可以访问"
        break
    fi
